---
title: "Daphnia kmer mutation study"
...
```
**This is the final analysis for submission.** 
GC correction bins: [0, 0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.75, 1.0]  
...
```
# Import data
This is already normalized by GC and depth by script `.py`  
Perform pre-processing on the data for later steps
```{r, load data}
...
# remove the technical replicates from this analysis for now, will come back to them later.
...
## Get common kmers  
Common kmers have a mean abundance of at least 100 copies across all lines.  
...
length(common.kmer.indexes) # number of total top kmers
...
#kmers.to.add <- common.kmer.names[!common.kmer.names %in% common.kmer.names34] # these are kmers that weren't in the top 34
#common.kmer.names34 [! common.kmer.names34 %in% common.kmer.names] # these are kmers that are no longer in the top 39 but were in the top 34
...
common.kmer.names[kb.kmer.indexes] # these are the super abundant kmers
```
##Get the legit kmers
...
for (i in 1:ncol(daphniakmers_normalized)) { ## go through each column of the data
  ...
  if (sum(p)==nrow(daphniakmers_normalized)) { # if all samples have at least 10 copies
...
#use the "legit" to add up the entire kmer content across lines
...
# note these are in Mb
total.kmers.abs
min(total.kmers.abs)
max(total.kmers.abs)
median(total.kmers.abs)
# return to this later when comparing MA lines with population

# are there any kmers 15 bp long?
...
```
## Output the kmers into a supplementary table
``` {r}
tableS1 <- data.frame(legit.kmer.names, colMeans(daphniakmers_normalized[,legit.kmer.indexes]))
#tableS1

#write.table(tableS1, file="~/Documents/FirstRotation-Andy/SuppTables/tableS1.txt" , quote=F, col.names=F, row.names=F, sep="\t")
```
## Figure 1
```{r}
...
Use kmers found in at least 100 copies in D. mel female (don't want to have Y-specific kmers included) 
Use script : `compare_dmel.py` to get intersecting kmers between D. mel and D. pul.  

Also look for common motifs: use script `.py` for D. pul, and `.py` for D. mel.
Python scripts were referenced.
```{r, dmel comparison}
# import data
dmelkmers <- read.delim("~/Documents/FirstRotation-Andy/dmel.normalizedkmers.txt") # already normalized
...
# Python script used to get all rotations of dmel kmers and see if they match to the D. pul kmers: 

# Python script used to fund motifs in D. pul kmers and D. mel kmers:
...
hist(daphniakmers_normalized[MA.indexes, 1], breaks=12)
hist(daphniakmers_normalized[MA.indexes, 7], breaks=12)
...
Pop.kmer.means < MA.kmer.means - (1*MA.kmer.sd)
```
...
Look across all kmers to see if this is indeed the case.  
...
plot(y=mutation.rate.abs, x=common.kmer.means[mutation.kmer.indexes], pch=19,  xlab="Mean abundance", ylab="Copies/generation")
...
```
## Output mutation rate into supplementary tables

``` {r}
#Table SX - mutation rates per kmer
#write.table()
...
## Minimum generations in the MA lines is implausible
Maybe get rid of this
```{r}
# in order for 20/39 kmers to have higher mutation rate in the 

# function that takes in a number of generations for the population, calculates the mutation rate for each kmer
# returns the number of kmers that the mutation rate is higher in the population

get_number_higher <- function (gen) {
  mutation.rate.abs.pop <- colMeans(kmer.deviation.abs[Pop.indexes, mutation.kmer.indexes])/(gen*initial_means[mutation.kmer.indexes])
  x <- length(which(mutation.rate.abs.pop > mutation.rate.abs.norm))
  return (x)
}
number_higher <- c()
for (i in 1:101) {
  number_higher <- c(number_higher, get_number_higher (i))
}
names(number_higher) <- c(1:101)
number_higher

# for half the kmers to have a higher mutation rate in the population than the MA lines, only 32-34 generations could have passed. This is very unlikely since Daphnia individuals only live up to 60 days and have a generation time of 10-14 days. When population isolates were taken from the tank, the MA lines were at generation 101.
...
# Fig 6C - check correlations between related kmers
related.kmer.indexes <- c(3,9,12,15,17,28,34,35,39)
temp <- kmer.deviation[MA.indexes,related.kmer.indexes]
colnames(temp) <- related.kmer.indexes
deviationCorr_MA_rel <- cor(temp)
corrplot(deviationCorr_MA_rel, type="upper", order="original", tl.col="black", tl.cex=1.0, tl.srt=0)
